 ...
 */
@Service
public class MchStlInfoServiceClient {

    @Autowired
    RestTemplate restTemplate;

    @HystrixCommand(fallbackMethod = "selectMchInfoByMchIdFallback")
    public String selectMchStlInfoByMchId(String jsonParam) {
        System.out.println("web中service参数：" + jsonParam);
        return restTemplate.getForEntity("http://EPAY-SERVICE/mch_info/selectMchStlInfoByMchId?jsonParam=" + MyBase64.encode(jsonParam.getBytes()), String.class).getBody();
    }
